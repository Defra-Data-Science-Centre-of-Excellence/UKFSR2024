...
t5_1_7i <- aws.s3::s3read_using(FUN = read_csv,
                                bucket = ukfsr::s3_bucket(),
                                object = "theme_5/t5_1_7/output/csv/5_1_7i_breakdown_hyg_compl_ratings_meat_estab.csv")

t5_1_7i$country <- factor(t5_1_7i$country, levels = c("England and Wales 2023","England and Wales 2022","Northern Ireland 2023","Northern Ireland 2022","Scotland 2023","Scotland 2022"))

t5_1_7i_long <- t5_1_7i %>% 
  group_by(country) %>%
  pivot_longer(cols=c("Good","Generally Satisfactory","Improvement Necessary","Urgent Improvement Necessary"),
               names_to="rating",
               values_to="value")

t5_1_7i_long$rating <- factor(t5_1_7i_long$rating, levels = c("Good","Generally Satisfactory","Improvement Necessary","Urgent Improvement Necessary"))
...
names(af_categorical_colours)=levels(t5_1_7i_long$rating)

t5_1_7i_long$country_wrap = str_wrap(t5_1_7i_long$country, width = 10)
...
t5_1_7i_long3 <- mutate(t5_1_7i_long1, foo = ifelse(value < 1.5, NA, value))
t5_1_7i_long4 <- mutate(t5_1_7i_long1, foo = ifelse(value < 1.5, value, NA))

t5_1_7i_plot <- ggplot(t5_1_7i_long3,aes(x=country_wrap, y=value, label = round(value,0), group=rating)) +
...
t5_1_7ii_plot <- ggplot(t5_1_7i_long4,aes(x=country_wrap, y=value, label = round(value,0), group=rating)) +
  geom_bar(stat="identity", aes(fill = rating)) +
  theme_ukfsr(base_family = "GDS Transport Website", base_size = 14) +
  scale_fill_manual(values = af_categorical_colours) +
  scale_y_continuous(limits = c(0,100.02),breaks = seq(0,100.02, 10))+
...
left1 <- filter(ggplot_build(t5_1_7ii_plot)$data[[2]],
...
right1 <- filter(ggplot_build(t5_1_7ii_plot)$data[[2]],
