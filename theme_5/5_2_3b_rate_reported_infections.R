...
t_5_2_3b <- aws.s3::s3read_using(FUN = read_csv,
                                 bucket = ukfsr::s3_bucket(),
                                 object = "theme_5/t5_2_3/output/csv/5_2_3b_rate_reported_infections_per_pop_per_year.csv")
...
t_5_2_3b$Year <- factor(t_5_2_3b$Year, levels = c("2019","2020","2021","2022","2023"))

t_5_2_3b_long <- t_5_2_3b |> 
  group_by(Year) |>
  pivot_longer(cols=c("Campylobacter sp.","Non typhoidal Salmonella sp.","STEC O157","Listeria monocytogenes"),
               names_to="species",
               values_to="value")

t_5_2_3b_long <- setDT(t_5_2_3b_long)
...
save_graphic(t_5_2_3b_long_plot_facet, "5.2.3b", "rate reported infections per pop per year")

save_csv(t_5_2_3b_long, "5.2.3b", "rate reported infections per pop per year")
