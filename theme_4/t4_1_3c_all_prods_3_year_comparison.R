...
# edited version
  ...
  group_by(cdid) %>%
  mutate(ten_yr_pct_change = (real - lag(real))/lag(real) * 100) %>%
  filter(date == as_date("2024-08-01")) %>%
  filter(cdid != "L522") %>%
  select(!c("date","value","real","period"))  %>%
  rename(`Percentage change (%)` = ten_yr_pct_change) %>%
...
save_graphic(t_4_1_3c_plot, "4.1.3c", "all prods 10 year comparison")

save_csv(t_4_1_3c_long, "4.1.3c", "all prods 10 year comparison")
