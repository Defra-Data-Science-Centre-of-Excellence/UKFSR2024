...
irrigation <- aws.s3::s3read_using(FUN = readr::read_csv,
                                   bucket = "s3-ranch-054",
...
# Plot the bar chart
irrigation_plot <- ggplot(irrigation, aes(x = Region, y = Volume / 1e3, fill = Type)) +
...
# NOT USED Map (Water Resource Availability) -----------------------------------
